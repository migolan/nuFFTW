...
struct nuFFT_tuner_t
{
	...
	nuFFT_implementation_t** p_implementations;
	int optimal_implementation_num;
	nuFFT_implementation_t* p_optimal_implementation;
	...
	nuFFT_tuner_t(int ndim, int* imagesize, DATA_T maxaliasingerror,
		DATA_T alphamin, DATA_T alphamax, int nalpha, resampling_method_t resampling_method, unsigned fftw_flags) :
			Ndim(ndim), Nimplementations(nalpha)
	{
		/*
			...
			linspace(alphamin, alphamax, Nalpha), so Nalpha also defines the number of implementations in the search space.
		*/
		...
		p_implementations = (nuFFT_implementation_t**) calloc (Nimplementations, sizeof(nuFFT_implementation_t*));
		DATA_T alphaspacing = (alphamax-alphamin)/nalpha;

		#pragma omp parallel for
		for (int iimp = 0; iimp < Nimplementations; iimp++) {
			double alpha = alphamin + iimp*alphaspacing;
			p_implementations[iimp] = (nuFFT_implementation_t*) calloc(1, sizeof(nuFFT_implementation_t));
			p_implementations[iimp]->set_params(Ndim, imagesize, maxaliasingerror, alpha, resampling_method);
		}
	...
	~nuFFT_tuner_t()
	{
		if (free_stuff) {
			free(trajectory);
			free(sqrtdcf);
			p_implementations[optimal_implementation_num]->Free();
		}
		for (int iimp=0; iimp<Nimplementations; iimp++)
			free(p_implementations[iimp]);
		free(p_implementations);
		free(N);
	}
	...
	int tune(tuning_heuristic_t tuning_heuristic, int nsamples, TRAJ_T* traj, TRAJ_T* sqrtdenscomp, unsigned fftw_flags, int npartitions)
	{
		Nsamples = nsamples;
		trajectory = traj;
	...
	{	
		...
		for (int iimp=0; iimp<Nimplementations; iimp++)
		{
			...
			if (tuning_heuristic == FFTWTUNE) {
				p_implementations[iimp]->tune_fftw(fftw_flags);
			
				for (int itrial=0; itrial < Ntrials; itrial++) {
					nuFFT_print("adjoint fftw: ");
					double t_nuFFT = p_implementations[iimp]->fftw(ADJOINT);
					t_nuFFT_avg += t_nuFFT;
				...
				if (t_nuFFT_avg < optimal_t_nuFFT) {
					if (optimal_implementation_num >= 0)
						p_implementations[optimal_implementation_num]->Free();
					optimal_t_nuFFT = t_nuFFT_avg;
					optimal_implementation_num = iimp;
				}
				else {
					p_implementations[iimp]->Free();
				}
				...
				p_implementations[iimp]->trajectory = trajectory;
				p_implementations[iimp]->sqrtdcf = sqrtdcf;
				p_implementations[iimp]->Nsamples = Nsamples;
				p_implementations[iimp]->compute();
				p_implementations[iimp]->tune(fftw_flags, npartitions);
			
			
				for (int itrial=0; itrial < Ntrials; itrial++)
					t_nuFFT_avg += p_implementations[iimp]->adjoint(noncartesian_data, cartesian_data);
				...
				if (t_nuFFT_avg < optimal_t_nuFFT) {
					if (optimal_implementation_num >= 0) {
						p_implementations[optimal_implementation_num]->trajectory = NULL;
						p_implementations[optimal_implementation_num]->sqrtdcf = NULL;
						p_implementations[optimal_implementation_num]->Free();
					}
					optimal_t_nuFFT = t_nuFFT_avg;
					optimal_implementation_num = iimp;
					...
					p_implementations[iimp]->trajectory = NULL;
					p_implementations[iimp]->sqrtdcf = NULL;
					p_implementations[iimp]->Free();
				}
			}
		}
		
		p_optimal_implementation = p_implementations[optimal_implementation_num];
		p_implementations[optimal_implementation_num] = NULL;
