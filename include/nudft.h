	...
	{	
		...
		for (int igrid=0; igrid<Ntotal; igrid++) {
			...
			for (int isample=0; isample<Nsamples; isample++) {
				...
				// Fourier integration
				noncartesian_data[isample][0] += sqrtdcf[isample] * (cartesian_data[igrid][0] * cos(phase) - cartesian_data[igrid][1] * sin(phase));
				noncartesian_data[isample][1] += sqrtdcf[isample] * (cartesian_data[igrid][0] * sin(phase) + cartesian_data[igrid][1] * cos(phase));
	...
	double adjoint (CPLX_T* noncartesian_data, CPLX_T* cartesian_data)
	{
		...
		for (int igrid=0; igrid<Ntotal; igrid++) {
			...
			for (int isample=0; isample<Nsamples; isample++) {
				...
				// Fourier integration
				cartesian_data[igrid][0] += sqrtdcf[isample] * (noncartesian_data[isample][0] * cos(phase) - noncartesian_data[isample][1] * sin(phase));
				cartesian_data[igrid][1] += sqrtdcf[isample] * (noncartesian_data[isample][0] * sin(phase) + noncartesian_data[isample][1] * cos(phase));
