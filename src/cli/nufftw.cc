...
int main(int argc, char* argv[])
{
	if (strcmp(argv[1],"-h") == 0) {
		nuFFT_print("nufftw_<typename> Nthreads Ndim imagesize maxerr_power alphamin alphamax nalpha resamplingmethod tuning_heuristic trajfilename\n");
		nuFFT_print("nufftw_<typename> Nthreads Ndim imagesize maxerr_power alphamin alphamax nalpha resamplingmethod tuning_heuristic trajfilename impfilename\n");
		return 0;
	}
	...
	DATA_T alphamin = strtod(argv[4+Ndim], NULL);
	DATA_T alphamax = strtod(argv[5+Ndim], NULL);
