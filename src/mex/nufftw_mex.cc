...
bool nuFFTW_mex_first_call = true;

void nuFFTW_mex_close(void)
{
	nuFFT_close();
	nuFFTW_mex_first_call = true;
}


void mexFunction (int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{	
	if (nuFFTW_mex_first_call) {
		nuFFTW_mex_first_call = false;
		mexAtExit(nuFFTW_mex_close);
	}


/*	
	imp = nufftw_mex(Nthreads, imagesize, maxerr, alphamin, alphamax, nalpha, resamplingmethod, tuning_heuristic, trajfilename);    (9)
		  nufftw_mex(Nthreads, imagesize, maxerr, alphamin, alphamax, nalpha, resamplingmethod, tuning_heuristic, trajfilename, impfilename); (10)
	imp = nufftw_mex(Nthreads, imagesize, maxerr, alphamin, alphamax, nalpha, resamplingmethod, tuning_heuristic, trajectory, sqrtdcf); (10)
		  nufftw_mex(Nthreads, imagesize, maxerr, alphamin, alphamax, nalpha, resamplingmethod, tuning_heuristic, trajectory, sqrtdcf, impfilename); (11)
*/
	...
	DATA_T alphamin = mxGetScalar(prhs[3]);
	DATA_T alphamax = mxGetScalar(prhs[4]);
	...
	nuFFT_tuner_t nuFFT_tuner (Ndim, imagesize, maxaliasingerror, alphamin, alphamax, Nalpha, resampling_method, fftw_flags);
	...
	if (nlhs == 0) { // write implementation to file
		...
		if (resampling_method == PRECOMPUTEDCONV && tuning_heuristic == FFTWTUNE)
			nuFFT_tuner.p_optimal_implementation->compute();
		...
		nuFFT_tuner.p_optimal_implementation->write_impfile(impfilename);
		...
		*((uint64_t *)mxGetData(plhs[0])) = reinterpret_cast<uint64_t>(nuFFT_tuner.p_optimal_implementation);
	}
}
