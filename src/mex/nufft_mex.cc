...
{	
	...
	if (strcmp(funname,"write_impfile") == 0) {
		/*	nufft_mex('write_nufft_impfile', Nthreads, imagesize, maxerr, alpha, resamplingmethod, trajectory, sqrtdcf, impfilename);
			nufft_mex('write_nufft_impfile', Nthreads, imagesize, maxerr, alpha, resamplingmethod, trajfilename, impfilename);
			nufft_mex('write_nufft_impfile', imp, impfilename);
		*/
		
		nuFFT_print("** write nuFFT implementation file**\n");
		...
		*/
		...
		nuFFT_print("** write nuFFT implementation **\n");
